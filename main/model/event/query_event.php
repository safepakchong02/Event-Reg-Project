...
use Nette\Utils\Random;
    ...
    switch ($value) {
        ...
        break;
        case "createEvent":
            $userId = $_POST['u_userId']; //CHAR(36) uuid
            $eventId = genEventId();
            $title = $_POST['ev_title']; //VARCHAR
            $detail = $_POST['ev_detail']; //VARCHAR
            $limit = $_POST['ev_limit']; //INT
            $dType = $_POST['ev_dType']; //VARCHAR
            $selfReg = $_POST['ev_selfReg']; //INT
            $preReg = $_POST['ev_preReg']; //INT
            $gps = $_POST['ev_gps']; //INT
            $lat = $_POST['ev_lat']; //DOUBLE
            $long = $_POST['ev_long']; //DOUBLE
            $preRegStart = $_POST['ev_preRegStart']; //DATETIME
            $preRegEnd = $_POST['ev_preRegEnd']; //DATETIME
            $checkInStart = $_POST['ev_checkInStart']; //DATETIME
            $checkInEnd = $_POST['ev_checkInEnd']; //DATETIME
            $eventStart = $_POST['ev_eventStart']; //DATETIME
            $eventEnd = $_POST['ev_eventEnd']; //DATETIME

            createEvent($userId, $eventId, $title, $detail, $limit, $dType, $selfReg, $preReg, $gps
            , $lat, $long, $preRegStart, $preRegEnd, $checkInStart, $checkInEnd, $eventStart
            , $eventEnd);
        break;
        case "editEvent":
            $eventId = $_POST['ev_eventId'];
            $title = $_POST['ev_title'];
            $detail = $_POST['ev_detail'];
            $limit = $_POST['ev_limit'];
            $dType = $_POST['ev_dType'];
            $selfReg = $_POST['ev_selfReg'];
            $preReg = $_POST['ev_preReg'];
            $gps = $_POST['ev_gps'];
            $lat = $_POST['ev_lat'];
            $long = $_POST['ev_long'];
            $preRegStart = $_POST['ev_preRegStart'];
            $preRegEnd = $_POST['ev_preRegEnd'];
            $checkInStart = $_POST['ev_checkInStart'];
            $checkInEnd = $_POST['ev_checkInEnd'];
            $eventStart = $_POST['ev_eventStart'];
            $eventEnd = $_POST['ev_eventEnd'];
            $status = $_POST['ev_status'];
            editEvent($eventId, $title, $detail, $limit, $dType, $selfReg, $preReg, $gps
            , $lat, $long, $preRegStart, $preRegEnd, $checkInStart, $checkInEnd, $eventStart
            , $eventEnd, $status);
        break;
        case "deleteEvent":
    ...
    function createEvent($userId, $eventId, $title, $detail, $limit, $dType, $selfReg, $preReg, $gps
    , $lat, $long, $preRegStart, $preRegEnd, $checkInStart, $checkInEnd, $eventStart
    , $eventEnd)
    {
    ...
    function editEvent($eventId, $title, $detail, $limit, $dType, $selfReg, $preReg, $gps
    , $lat, $long, $preRegStart, $preRegEnd, $checkInStart, $checkInEnd, $eventStart
    , $eventEnd, $status)
    {
        $handle->begin_transaction();
        try {
            $query = "CALL editEvent( " . $eventId . ", " . $title . ", 
            " . $detail . ", " . $limit . ", 
            " . $dType . ", " . $selfReg . ", 
            " . $preReg . ", " . $gps . ",
            " . $lat . ", " . $long . ", 
...
/*if (@$_GET['event_view'] != '') {
 $event_view = @$_GET['event_view']; } else if (@$_POST['event_view'] != '') {
 $event_view = @$_POST['event_view']; } else {
 $event_view = ''; } include_once('../../../asset/config/config.php');
 //******************** show data *********************/
/*if (@$_SESSION["perm"] == "") {
 echo "{\"status\": 403}"; } else if ($event_view == 'show_data') {
...
/*} else if ($event_view == 'add') {
 ...
 //******************** view Edit *********************//* } else if ($event_view == 'show_data_edit') {
 ...
 echo $results;
 //******************** Update Edit *********************//* } else if ($event_view == 'edit_form_save') {
 if ($_POST['ev_id'] != '') {
 $ev_id = $_POST["ev_id"];
 $ev_title = $_POST['ev_title'];
 $ev_date_start = $_POST['ev_date_start'];
 ...
 //******************** delete data *********************//* } else if ($event_view == 'del_event') {
 if ($_GET['ev_id'] != '') {
 $ev_id = $_GET['ev_id'];
